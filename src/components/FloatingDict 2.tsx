...
export default function ArticlePage({
  ...
  // --- data (local import for reliability) ---
          ...
          <div className="news-frame p-6 space-y-3">
            ...
            {!article ? (
              <div className="text-sm text-red-600">
                Article <code>{params.id}</code> not found. Check <code>articles.json</code>.
              </div>
            ) : (
              <div className="text-sm text-red-600">
  ...
  // --- Floating dictionary (manual open only) ---
  const [floatOpen, setFloatOpen] = useState(false);
  const [dictQuery, setDictQuery] = useState("");
  const [selectedText, setSelectedText] = useState("");

  // Keep track of OS selection, but DO NOT auto-open
  useEffect(() => {
    const readSelection = () => {
      const txt = (document.getSelection?.()?.toString() || "").trim();
      setSelectedText(txt);
    };
  ...
  // Open dictionary using whatever is currently selected
  function openDictFromSelection() {
    const txt = selectedText.trim();
    if (!txt) return;
    setDictQuery(txt);
    setFloatOpen(true);
    // optionally clear native selection highlight
    try { document.getSelection?.()?.removeAllRanges(); } catch {}
  }

  // Keyboard shortcut: ⌘/Ctrl + D to open dictionary with current selection
  useEffect(() => {
  ...
  // Collect vocab for local-first lookup inside the popup
  const vocabList =
  ...
  // --- Glossary (persist in localStorage) ---
