...
export default function ArticlePage({
  ...
  /* --- data --- */
  const articles = (data as any[]) as Article[];
    ...
    return (
      <div className="min-h-screen bg-gray-100">
          ...
          <div className="mx-auto max-w-4xl px-6">
            <div className="flex items-center justify-between py-4">
              <div className="flex items-center gap-3">
                <div className="h-6 w-6 rounded-sm bg-[var(--nt-accent)]" />
        ...
        <main className="page-wrap py-8 space-y-6">
          <div className="news-frame p-6 space-y-3">
            <h2 className="text-xl font-semibold">Article Detail</h2>
            {!article ? (
              <div className="text-sm text-red-600">
                Article <code>{params.id}</code> not found. Check your <code>articles.json</code>.
              </div>
            ) : (
              <div className="text-sm text-red-600">
                Segment <code>{params.segmentId}</code> not found in article <code>{params.id}</code>.
              </div>
            )}
            <Link href="/" className="text-sm text-blue-600 hover:underline">← Back to Index</Link>
          </div>
        </main>
      </div>
    );
  ...
  /* --- Azure TTS: Model speech --- */
  const [isTtsLoading, setIsTtsLoading] = useState(false);
  const [ttsUrl, setTtsUrl] = useState<string | null>(null);

  function clearSelectionSafely() {
    try {
  ...
  function pauseRecording() {
    ...
    try { mr.pause(); setIsPaused(true); } catch {}
  }
  function resumeRecording() {
    clearSelectionSafely();
    const mr = mediaRecorderRef.current;
    if (!mr || !isRecording || !isPaused) return;
    try { mr.resume(); setIsPaused(false); } catch {}
  }
  ...
  // track current OS selection so we can show the button only when there IS a selection
  ...
  useEffect(() => {
    ...
    document.addEventListener("touchend", handler, { passive: true } as any);
    return () => {
  ...
  return (
    <div className="min-h-screen bg-gray-100">
        ...
        <div className="mx-auto max-w-4xl px-6">
          <div className="flex items-center justify-between py-4">
            ...
            <div className="hidden sm:flex items-center gap-3 text-xs text-gray-300">
              <span>EN / JA</span><span>•</span><span>JHS / HS</span>
            </div>
          </div>
        </div>
      </header>
      ...
      {/* Main */}
      <main className="page-wrap py-8 space-y-6">
        {/* Back to Index at the top */}
        <div className="mx-auto max-w-4xl px-6">
          <Link href="/" className="text-sm text-blue-600 hover:underline">← Back to Index</Link>
        </div>

        {/* Article metadata */}
        <section className="news-frame p-6 space-y-4 mx-auto max-w-4xl">
          <p className="text-xs text-gray-500 uppercase tracking-wider">
            {segment.kicker || "News"} ·{" "}
            {article.publishedAt ? new Date(article.publishedAt).toLocaleString() : "No date"}
          </p>
          <h2 className="text-3xl font-extrabold leading-tight">{displayTitle}</h2>
          {segment.headline_ja && <h3 className="text-lg text-gray-700">{segment.headline_ja}</h3>}
          <p className="text-sm text-gray-600">Source: {article.source || "—"}</p>
        </section>

        {/* Toggles */}
        <section className="card p-4 flex flex-wrap items-center gap-4 mx-auto max-w-4xl">
          <div className="flex items-center gap-2">
            <label className="text-sm font-medium">Level</label>
            <select
              className="rounded-md border px-2 py-1 bg-white"
              value={level}
              onChange={(e) => { clearSelectionSafely(); setLevel(e.target.value as "JHS" | "HS"); }}
            >
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              className="rounded-md border px-2 py-1 bg-white"
              value={lang}
              onChange={(e) => { clearSelectionSafely(); setLang(e.target.value as "en" | "ja"); }}
            >
          ...
          <label className="ml-auto inline-flex items-center gap-2 text-sm font-medium">
            <input
              type="checkbox"
              checked={showEasy}
              onChange={(e) => { clearSelectionSafely(); setShowEasy(e.target.checked); }}
            />
            Easy Mode
          </label>
        </section>
        ...
        {/* Main article */}
        <article className="card p-6 prose prose-news max-w-none select-text mx-auto max-w-4xl">
        ...
        {showEasy && variant?.easy && (
          <section className="card p-4 border-l-4 border-l-blue-400 bg-blue-50 mx-auto max-w-4xl">
        ...
        {/* Model Speech */}
        <section className="card p-4 space-y-3 mx-auto max-w-4xl">
        ...
        {/* Glossary preview */}
        <section className="card p-4 space-y-2 mx-auto max-w-4xl">
        ...
        <footer className="pt-2 pb-8 text-center text-gray-600 text-sm mx-auto max-w-4xl">
          <div className="mb-3">
            <Link href="/" className="text-sm text-blue-600 hover:underline">← Back to Index</Link>
          </div>
      ...
      {/* Manual “Look up selection” button — appears only when there IS a selection */}
      ...
      <FloatingDict
        ...
        vocabList={vocabList}
        onSaveGlossary={(item) =>
          handleSaveGlossary({ term: item.term, def_en: item.def_en, ja: item.ja, source: item.source })
        }
      />
    </div>
  );
